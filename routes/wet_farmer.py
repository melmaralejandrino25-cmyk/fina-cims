...
wet_farmer_bp = Blueprint("wet_farmer", __name__)


# =========================
# LIST FARMERS
# =========================
@wet_farmer_bp.route("/<int:assoc_id>")
@login_required
def list_farmers(assoc_id):
    ...
    association = db.execute(
        "SELECT * FROM wet_associations WHERE id = ?", (assoc_id,)
    ).fetchone()
    ...
    if search:
        query = """
            SELECT * FROM wet_farmers
            WHERE association_id = ?
              AND (rsbsa LIKE ? OR last_name LIKE ? OR first_name LIKE ?)
            ORDER BY last_name, first_name
        """
        search_param = f"%{search}%"
        farmers = db.execute(query, (assoc_id, search_param, search_param, search_param)).fetchall()
    else:
        farmers = db.execute(
            "SELECT * FROM wet_farmers WHERE association_id = ? ORDER BY last_name, first_name",
            (assoc_id,)
        ).fetchall()

    varieties = db.execute("SELECT name FROM wet_varieties ORDER BY name").fetchall()
...
# ADD FARMER
# =========================
@wet_farmer_bp.route("/add/<int:assoc_id>", methods=["POST"])
@login_required
...
def add_farmer(assoc_id):
    ...
    return redirect(url_for("wet_farmer.list_farmers", assoc_id=assoc_id))


# =========================
# EDIT FARMER
# =========================
@wet_farmer_bp.route("/edit/<int:id>", methods=["GET", "POST"])
@login_required
@role_required("admin", "encoder")
def edit_farmer(id):
    db = get_db()

    farmer = db.execute("SELECT * FROM wet_farmers WHERE id = ?", (id,)).fetchone()
    if not farmer:
    ...
    if request.method == "POST":
        ...
        return redirect(url_for("wet_farmer.list_farmers", assoc_id=farmer["association_id"]))

    return render_template("wet/edit_farmer.html", farmer=farmer)


# =========================
# DELETE FARMER
# =========================
@wet_farmer_bp.route("/delete/<int:id>")
@login_required
@role_required("admin")
def delete_farmer(id):
    db = get_db()

    farmer = db.execute("SELECT association_id FROM wet_farmers WHERE id = ?", (id,)).fetchone()
    if not farmer:
        return "Farmer not found", 404

    assoc_id = farmer["association_id"]

    db.execute("DELETE FROM wet_farmers WHERE id = ?", (id,))
    db.commit()
...
# UPLOAD EXCEL / CSV
# =========================
@wet_farmer_bp.route("/upload_excel/<int:assoc_id>", methods=["POST"])
@login_required
@role_required("admin", "encoder")
def upload_excel(assoc_id):
    ...
    for _, row in df.iterrows():
        ...
        if rsbsa:
            duplicate = db.execute(
                "SELECT 1 FROM wet_farmers WHERE LOWER(TRIM(rsbsa)) = LOWER(TRIM(?)) LIMIT 1",
                (rsbsa,)
            ).fetchone()
    ...
    if duplicates > 0:
        flash(
            f"Wet Season upload complete: {inserted} farmer(s) imported; {duplicates} duplicate(s) skipped.",
            "warning"
        )
    else:
        flash(f"Wet Season upload complete: {inserted} farmer(s) imported successfully.", "success")
...
# =========================
@wet_farmer_bp.route("/dashboard/<int:assoc_id>")
@login_required
def association_dashboard(assoc_id):
    ...
    return render_template(
        "wet/association_dashboard.html",
        association=association,
...
# =========================
@wet_farmer_bp.route("/export/<int:assoc_id>")
@login_required
def export_farmers(assoc_id):
    ...
    return send_file(
        output,
        download_name=f"wet_farmers_assoc_{assoc_id}.xlsx",
        as_attachment=True,
